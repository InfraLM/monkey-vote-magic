...
export const ThankYouPage = ({ onContinueVoting }: ThankYouPageProps) => {
    ...
    <div className="min-h-screen bg-gradient-to-br from-starburst-from to-starburst-to relative overflow-hidden">
      ...
      <div className="fixed inset-0 pointer-events-none opacity-50 z-0">
        ...
        <img 
          ...
          alt="" 
          className="absolute top-40 right-4 w-28 sm:w-36 md:w-48 -rotate-12 transition-transform" 
          style={{ transform: `translateY(${scrollY * 0.15}px) rotate(-12deg)` }}
        />
        <img 
          src={sticker14Dez} 
          alt="" 
          className="absolute top-[60%] left-8 w-20 sm:w-28 md:w-36 rotate-6 transition-transform" 
          style={{ transform: `translateY(${scrollY * 0.08}px) rotate(6deg)` }}
        />
        <img 
          src={stickerHop} 
          alt="" 
          className="absolute bottom-20 right-8 w-24 sm:w-32 md:w-44 -rotate-6 transition-transform" 
          style={{ transform: `translateY(${scrollY * -0.12}px) rotate(-6deg)` }}
        />
        <img 
          src={stickerTv} 
          alt="" 
          className="absolute bottom-40 left-4 w-24 sm:w-32 md:w-40 rotate-45 transition-transform" 
        ...
        <img 
          ...
          alt="" 
          className="absolute top-[30%] left-[10%] w-20 sm:w-24 md:w-32 rotate-[20deg] transition-transform" 
          style={{ transform: `translateY(${scrollY * 0.2}px) rotate(20deg)` }}
        />
        <img 
          src={stickerPixel} 
          alt="" 
          className="absolute top-[15%] right-[15%] w-16 sm:w-20 md:w-28 -rotate-12 transition-transform" 
          style={{ transform: `translateY(${scrollY * 0.18}px) rotate(-12deg)` }}
        />
        <img 
          src={stickerSoldout} 
          alt="" 
          className="absolute top-[75%] left-[5%] w-20 sm:w-24 md:w-32 rotate-12 transition-transform" 
          style={{ transform: `translateY(${scrollY * -0.15}px) rotate(12deg)` }}
        />
        <img 
          src={stickerVenenosa} 
          alt="" 
          className="absolute top-[50%] right-[8%] w-20 sm:w-28 md:w-40 -rotate-[30deg] transition-transform" 
          style={{ transform: `translateY(${scrollY * 0.13}px) rotate(-30deg)` }}
        />
        <img 
          src={stickerEngenharia} 
          alt="" 
          className="absolute bottom-[15%] right-[20%] w-20 sm:w-24 md:w-32 rotate-[15deg] transition-transform" 
      ...
      <div className="max-w-3xl mx-auto px-4 py-8 md:py-12 relative z-10">
        ...
        <div className="text-center mb-8 md:mb-12 animate-fade-in">
          <div className="mb-6 flex justify-center">
            <img 
              src={logoMain} 
              alt="MDA 2025" 
              className="w-32 h-32 sm:w-40 sm:h-40 md:w-48 md:h-48 drop-shadow-2xl animate-scale-in" 
            />
          </div>
        ...
        <Card className="p-6 sm:p-8 md:p-10 mb-6 shadow-pop border-4 border-accent bg-card/95 backdrop-blur-sm">
          <div className="text-center space-y-6">
            <h2 className="text-2xl sm:text-3xl md:text-4xl font-black text-accent uppercase">
              Agora garanta seu ingresso! 🌅🎉
            </h2>
            
            <p className="text-base sm:text-lg md:text-xl font-semibold text-foreground leading-relaxed">
              Não perca a <span className="text-primary font-black">SUNSET + RESENHA DO MUNDINHO UFG</span>! Venha celebrar com a gente no evento mais aguardado de 2025!
            </p>

            <Button 
              asChild 
              size="lg" 
              className="w-full h-14 sm:h-16 md:h-20 text-xl sm:text-2xl md:text-3xl font-black uppercase shadow-pop hover:scale-105 transition-transform"
            >
              <a 
                href="https://cheers.com.br/evento/mda-2025-28126" 
                target="_blank" 
        ...
        <div className="text-center">
          <Button 
            ...
            variant="secondary"
            size="lg"
            className="w-full sm:w-auto px-8 h-12 sm:h-14 text-lg sm:text-xl font-bold uppercase shadow-sticker hover:scale-105 transition-transform"
          >
